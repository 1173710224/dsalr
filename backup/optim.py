...
class MiniDsa(Optimizer):
    # 加入对历史信息的考量，可以缓解minibatch下对局部特征过于敏感的问题
...
class FDecreaseDsa(Optimizer):
    # 加入慢增快减，当学习率需要下降时，可以快速地将学习率调整至一个较小值
...
class MixDsa(Optimizer):
    # 慢增快减+历史信息，混合优化，前半段使用minibatch，后半段使用batch
